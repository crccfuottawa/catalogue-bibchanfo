...
function applyFilters() {
  ...
  filteredRecords = records.filter(record => {
    ...
    const recordEnd =
      yearFrom(record.DateFin || record.Date) ?? recordStart;
...
function addClickableAuthorDetail(dl, value) {
  if (!value) return;
...
function renderRecords() {
  ...
  pageRecords.forEach(record => {
    ...
    card.querySelector(".record-type").textContent =
      record.Type || "Notice";
    ...
    addClickableAuthorDetail(dl, record.Auteur);
    addDetail(dl, "Périodique", record["Titre Périodique"]);
    addDetail(
      dl,
      "Date",
      record.Date ||
        [record.DateDébut, record.DateFin].filter(Boolean).join(" – ")
    );
    addDetail(
      dl,
      "Volume / numéro",
...
async function init() {
  try {
    ...
    const types = [
      ...new Set(
        records
          .flatMap(record => splitOptions(record.Type))
          .filter(Boolean)
      )
